...
const SingleProduct = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const similar = await axios.get(`https://fakestoreapi.com/products/category/${data.category}`);
        const filt = similar.data.filter((product) => product.id !== data.id);
        setSimilarData(filt);
      } catch (err) {
        setError(err);
      } finally {
  ...
  if (loading) return (<>
    <div className=' flex justify-center items-center'><Image width={600} height={315} src='/images/lg.gif' alt="loading" className=' ' /></div>
  </>);
                ...
                <Link href={`/singleproduct/${product.id}`}>
                  ...
                  <div className="flex justify-between items-center px-1 mt-2">
                    ...
                    <h4 className="font-bold text-[16px]">
                      ...
                      <span className="line-through text-gray-500">$150.0</span>
                    </h4>
                  </div>
